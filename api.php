...
function get_playlist_info($playlist_id)
{
    ...
    // $url = "http://music.163.com/api/music/playlist?id=" . $playlist_id;
    return curl_get($url);
}

//获取热门歌单
function get_hot_playlist($limit,$type){
    // $url="http://music.163.com/discover/playlist/?limit=".$limit.'&order='.$type;
    $url="http://music.163.com/api/playlist/list?cat=全部&offset=0&limit=".$limit."&order=".$type;
    return curl_get($url);
}

//获取新专辑
function get_new_ablum($limit){
    $url="http://music.163.com/api/album/new?area=ALL&offset=0&total=true&limit=".$limit;
    return curl_get($url);
}
...
switch ($API_type) {
	...
	echo get_mv_info($queryString['id']);
        break;
    case 'get_hot_playlist':
    echo get_hot_playlist($queryString['limit'],$queryString['type']);
        break;
    case 'get_new_ablum':
    echo  get_new_ablum($queryString['limit']);
        break;
	default:
		echo "SWITCH ERROR";
		break;
